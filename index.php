    ...
    <ul>
      <?php
		...
		foreach($files1 as $indx=>$name)
		{
			$count=$count+1;
			if($count<=5){
				echo "<li><a href='caption.php?image=$name&id=0'><img src='caption_images/$name' alt=''/></a></li>";
			}
		}
...
if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "bmp" && $imageFileType != "tiff" && $imageFileType != "jfif") {
    echo "<script>alert('Sorry, only JPG, JPEG, PNG, BMP, JFIF and TIFF files are allowed.')</script>";
    $uploadOk = 0;
}
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        echo "<script>alert('The file has been uploaded successfully.');window.location.href='caption.php?image=$name&id=1';</script>";
